...
my $home = glob('~') . '/';
my @dirs = qw(rakudo-m-inst-1 rakudo-m-inst-2);
my %swap = (@dirs, reverse(@dirs));

my $link = 'rakudo-m-inst';
...
if (not -d $dirs[0] or not -d $dirs[1]) {
    for my $d (@dirs) {
        mkdir $d, 0777;
        (my $s = $d) =~ s/-inst//;
        system 'git', 'clone', '--recursive', 'https://github.com/rakudo/rakudo.git', $s;
    }
    symlink $dirs[1], $link;
}
